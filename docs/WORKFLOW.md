...
```
 [1] 설계·검수 세션(Claude) / spec-writer      계획 → docs/specs/NNN-<slug>.md
          │
          ▼
 [2] 구현 세션 / implementer                   스펙 → 코드 + 테스트 실행
          │
          ▼
 [3] 설계·검수 세션(Claude) / impl-reviewer    git diff + 스펙 대조 → docs/reviews/NNN-<slug>.md
          │
...
| # | 어디서 | 정의 | 산출물 | 하지 않는 것 |
|---|---|---|---|---|
| 1 | 설계·검수 세션(Claude) | [`spec-writer`](agents/spec-writer.md) | `docs/specs/NNN-*.md` | 구현 코드를 쓰지 않는다 |
| 2 | 구현 세션 | [`implementer`](agents/implementer.md) | 소스 + 테스트 | 설계 판단, 범위 밖 수정, 커밋 |
| 3 | 설계·검수 세션(Claude) | [`impl-reviewer`](agents/impl-reviewer.md) | `docs/reviews/NNN-*.md` | 코드를 고치지 않는다 |
| 4 | 사람 | — | 커밋 | — |

## 파일 위치

에이전트 지침은 **`docs/agents/` 한 곳에만** 둔다. 이 폴더는 도구 중립적이라
어떤 세션(LLM 백엔드)에서도 같은 경로로 읽는다. 문서는 전부 `docs/` 아래에 모은다.
...
docs/
  ...
  specs/
    TEMPLATE.md               스펙 양식
    NNN-<slug>.md             [1] 산출물 → [2] 입력 → [3] 대조 기준
  reviews/
    NNN-<slug>.md             [3] 산출물 → 사람이 커밋 전에 읽음
.claude/                      Claude Code 전용 — git 에 올리지 않는다 (.gitignore)
  agents/
    spec-writer.md            → docs/agents/spec-writer.md 를 읽게 전달만 한다
    impl-reviewer.md          → docs/agents/impl-reviewer.md 를 읽게 전달만 한다
```

`.claude/agents/` 는 Claude Code 가 서브에이전트를 자동 등록하기 위한 규약 폴더일 뿐,
**실제 내용은 갖지 않는다.** 지침을 고칠 때는 `docs/agents/` 만 고친다.

`docs/agents/implementer.md` 는 **서브에이전트로 spawn 하지 않는다.**
...
```
docs/specs/003-guard.md 를 구현하라. docs/agents/implementer.md 의 규칙을 따르라.
```
...
| `guard.py` (SQL 안전성 검사) | 완료 — 리뷰 PASS (`docs/reviews/002-guard.md`) |
| `db.py` / `schema.py` | 미착수 |
| `graph.py` / `cli.py` | 미착수 |
| 평가 · 쿼리 효율성 (v2) | 미착수 |